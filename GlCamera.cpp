...
GlCamera::AimInfo::AimInfo(const vec3& p, const vec3& t, const vec3& u):
  position(p), target(t), up(u) {}

vec3 GlCamera::unproject(int x, int y, float z) const {
  return unproject(vec3(x, _viewport[3]-y-1, z));
}

vec3 GlCamera::unproject(const vec3& wincoords) const {
...
void GlCamera::AimInfo::translate(const vec3& v) {

  Transform3 t = xform();
  vec3 vt = t.rotation().inverse() * v;
  position += vt;
  target += vt;

}

void GlCamera::AimInfo::rotate(const quat& q) {

  float d = (position-target).norm();

  Transform3 told = xform();

  quat qnew = q * told.rotation();
  mat3 Rnew = qnew.matrix();
    
  vec3 fwd(Rnew(2,0), Rnew(2,1), Rnew(2,2));
  position = target + fwd * d;

  up = vec3(Rnew(1,0), Rnew(1,1), Rnew(1,2));

}

Transform3 GlCamera::AimInfo::xform() const {

  const vec3& p = position;

  quat q = quatFromTwoVectors( up, p-target );
  
  return Transform3(q, p).inverse();
...
void GlCamera::zoom(float factor) {
  ...
  vec3 diff = _current.aim.position - _current.aim.target;
  diff /= diff.norm();
...
void GlCamera::setOrtho(const vec3& viewDims) {
...
void GlCamera::aim(const vec3& position,
                   const vec3& target,
                   const vec3& up) {
...
const vec3& GlCamera::orthoViewDims() const {
...
const mat4& GlCamera::getMatrix(MatrixType m) const {
  switch (m) {
...
const mat4& GlCamera::projection() const { return _projection; }

const mat4& GlCamera::modelview() const { return _modelview; }

const mat4& GlCamera::projectionInverse() const { return _projection_inv; }

const mat4& GlCamera::modelviewInverse() const { return _modelview_inv; }
...
vec3 GlCamera::_hemiCoords(int x, int y) const {
...
void GlCamera::mouseMove(int x, int y, MouseMode m) {
  ...
  case MOUSE_ROTATE:
    if (_rotateType == ROTATE_2_AXIS) {
      ...
      vec3 h1 = _hemiCoords(x, y);
      vec3 axis = _mouseHemi.cross(h1);
      float st = axis.norm();
      float ct = _mouseHemi.dot(h1);
      if (st  > 1e-5) {
        axis /= st;
        float angle = acos(std::min(1.0f, std::max(ct, -1.0f)));
        _current = _mouseOrigPose;
        _current.aim.rotate(quat(Eigen::AngleAxisd(angle, axis)));
        _updateModelview();
      }
    }
    break;
...
void GlCamera::pan(float px, float py, float pz, bool autoscale) {
  pan(vec3(px,py,pz),autoscale);
}

void GlCamera::pan(const vec3& p, bool autoscale) {
  float s = 1;
  if (autoscale) {
    int psz = std::min(_viewport[2], _viewport[3]);
    s = _winsz / psz;
  }
  vec3 offs = _cxform.rotation().inverse() * (s * p);
...
void GlCamera::_updateProjection() {
  ...
  if (_cameraType == CAMERA_PERSPECTIVE) {
    ...
    const vec3& vdims = _current.orthoViewDims;
...
void GlCamera::_updateModelview() {
  ...
  if (_rotateType == ROTATE_2_AXIS) {
    ...
    quat rx = quatFromAxisAngle(vec3(1,0,0), _current.xrot * M_PI / 180);
    quat ry = quatFromAxisAngle(vec3(0,1,0), _current.yrot * M_PI / 180);
    tmp.rotate(rx * ry);
    _cxform = tmp.xform();
  } else {
...
const Transform3& GlCamera::xform() const {
  return _cxform;
}
